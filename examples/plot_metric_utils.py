...
def plot_Nckpts_1leadtime(leadtime = 5, ckpts=[1,30,70,90], experiment_number=1, 
                    ...
                    plot_truth_contour=False, save_fig=True, figs_dir = 'DEFINE_INFERENCE_RUN_DIRECTORY',
                    title=None,
                    ):
    ...
    # Load Truth Data
    truth_fp = f"DEFINE_INITIALIZATION_DIRECTORY/Initialize_{init_timestep_str}_nsteps{leadtime*4}.nc"
    ...
    if error:
        ...
        for ckpt_num in ckpts:
            output_fp = f"DEFINE_INFERENCE_RUN_DIRECTORY/Experiment{experiment_number}/{valid_timestep_str[:10]}/Checkpoint{ckpt_num}_{init_timestep_str}_nsteps{leadtime*4}.nc"
            ds = xr.open_dataset(output_fp)
...
def plot_mse_ckpt_leadtimes(ckpts, var, experiment_number, highlight_min = True, mses=None,
                figs_dir = 'DEFINE_PROJECT_DIRECTORY/figures', bounding_box={},
...
def plot_iou_ckpt_leadtimes(ckpts, leadtimes, var, experiment_number, valid_timestep_str, bounding_box, 
        contour_percentile=80, figs_dir = 'DEFINE_PROJECT_DIRECTORY/figures', highlight_max=True,
        presentation_style=False, sigma=1):
    ...
    for leadtime in [3,5,7]:
        ...
        if presentation_style:
            ...
            # Add smoothed line on top
            ious_smooth = gaussian_filter(ious_select, sigma=sigma)
            ax.plot(ckpts, ious_smooth, label=f'{leadtime}-day leadtime', color=colors[leadtime], alpha=0.9, linewidth=3)
        else:
    ...
    if presentation_style:
        ...
        ax.set_title(f'{valid_timestep_str[:10]} Atmospheric River\nLocation Accuracy throughout Training')
        plt.ylim(0,0.9)
    else:
        ax.set_ylabel("IoU")
...
def plot_err_amp_ckpts_leadtimes(ckpts, leadtimes, var, experiment_number, valid_timestep_str, bounding_box, 
        contour_percentile=80, figs_dir = 'DEFINE_PROJECT_DIRECTORY/figures', highlight_min=True,
